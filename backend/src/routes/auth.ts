...
router.get('/me', verifyToken, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(
      `SELECT id, email, username, oauth_provider, avatar_url, created_at
...
router.get('/github/callback', async (req, res) => {
  try {
    ...
    const { accessToken, profile } = await exchangeGitHubCode(code as string);

    // Check if user exists
    let userResult = await query(
      `SELECT id FROM users WHERE oauth_provider = 'github' AND oauth_id = $1`,
      [profile.id]
    );
    ...
    if (userResult.rows.length > 0) {
      ...
      await query(
        `INSERT INTO users (id, email, username, oauth_provider, oauth_id, avatar_url, created_at, updated_at)
         VALUES ($1, $2, $3, 'github', $4, $5, NOW(), NOW())`,
        [userId, profile.email, profile.login, profile.id, profile.avatar_url]
      );
    }
    ...
    logger.info('GitHub OAuth successful', { username: profile.login, userId });
...
router.post('/gitlab', async (req, res) => {
  try {
    ...
    logger.error('Error initiating GitLab OAuth:', error);
    res.status(500).json({ error: 'Failed to initiate OAuth' });
  }
});
...
router.get('/gitlab/callback', async (req, res) => {
  try {
    ...
    const { accessToken, profile } = await exchangeGitLabCode(code as string);

    let userResult = await query(
      `SELECT id FROM users WHERE oauth_provider = 'gitlab' AND oauth_id = $1`,
      [profile.id]
    );
    ...
    if (userResult.rows.length > 0) {
      ...
      await query(
        `INSERT INTO users (id, email, username, oauth_provider, oauth_id, avatar_url, created_at, updated_at)
         VALUES ($1, $2, $3, 'gitlab', $4, $5, NOW(), NOW())`,
        [userId, profile.email, profile.username, profile.id, profile.avatar_url]
      );
    }
    ...
    logger.info('GitLab OAuth successful', { username: profile.username, userId });
...
router.post('/token/verify', verifyToken, (req: AuthRequest, res) => {
  try {
    res.status(200).json({
      valid: true,
      user: req.user,
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    const userResult = await query(
      'SELECT id, email, username, oauth_provider FROM users WHERE id = $1',
      [tokenData.id]
    );
...
router.get('/callback/status', (req, res) => {
  try {
    ...
    const callbackData = (req as any).oauthCallback;
    ...
    if (callbackData && callbackData.provider === provider) {
      ...
      const response = {
        ...
        state: oauthState[provider as string] || null,
        user: callbackData.user
      };

      // Clear the callback data
      delete (req as any).oauthCallback;
    ...
    logger.error('Callback status check failed:', error);
