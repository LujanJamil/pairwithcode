...
router.post('/comments', async (req, res) => {
    try {
        ...
        const comment = await code_review_1.codeReviewService.createComment({
            sessionId,
            filePath,
            lineNumber,
            userId,
        ...
        logger_1.logger.error('Error creating code review comment:', error);
...
router.post('/comments/:commentId/replies', async (req, res) => {
    try {
        ...
        const reply = await code_review_1.codeReviewService.addReply({
            commentId,
            userId,
            userName: userName || 'Anonymous',
            content
        ...
        logger_1.logger.error('Error adding code review reply:', error);
...
router.get('/sessions/:sessionId/comments', async (req, res) => {
    try {
        ...
        const comments = await code_review_1.codeReviewService.getSessionComments(sessionId);
        res.status(200).json({
            sessionId,
            comments,
        ...
        logger_1.logger.error('Error fetching session comments:', error);
...
router.get('/sessions/:sessionId/files/:filePath/comments', async (req, res) => {
    try {
        ...
        const comments = await code_review_1.codeReviewService.getFileComments(sessionId, decodeURIComponent(filePath));
        res.status(200).json({
            sessionId,
            filePath,
            comments,
        ...
        logger_1.logger.error('Error fetching file comments:', error);
        res.status(500).json({ error: 'Failed to fetch comments' });
    }
});
...
router.patch('/comments/:commentId/status', async (req, res) => {
    try {
        ...
        await code_review_1.codeReviewService.updateCommentStatus(commentId, status);
        res.status(200).json({ success: true, status });
    }
    catch (error) {
        logger_1.logger.error('Error updating comment status:', error);
...
router.delete('/comments/:commentId', async (req, res) => {
    try {
        const { commentId } = req.params;
        await code_review_1.codeReviewService.deleteComment(commentId);
...
router.get('/sessions/:sessionId/report', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const report = await code_review_1.codeReviewService.generateReviewReport(sessionId);
        res.status(200).json({
            sessionId,
            ...report
        });
        ...
        logger_1.logger.error('Error generating review report:', error);
