...
class AnalyticsService {
    ...
    async recordEdit(sessionId, userId, filePath) {
        const query = `
    ...
    async recordConflict(sessionId, userId, filePath) {
        const query = `
      INSERT INTO analytics_events (session_id, user_id, event_type, file_path, timestamp)
      VALUES ($1, $2, $3, $4, NOW())
    `;
    ...
    async getTimelineMetrics(sessionId, intervalSeconds = 60) {
        const query = `
      SELECT 
    ...
    async getUserActivity(sessionId) {
        const query = `
      SELECT 
        u.id,
        u.name,
